...
def geometry_type(kind: str, *, spatial_index: bool = False) -> Any:
    if get_settings().database_url.startswith("sqlite"):
        return Text()
    return Geometry(kind, srid=4326, spatial_index=spatial_index)
...
class WatchArea(TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(160))
    geometry: Mapped[Any] = mapped_column(geometry_type("POLYGON", spatial_index=True))
...
class Observation(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "source",
            "source_item_id",
            name="uq_observation_area_provider_source",
        ),
        CheckConstraint(
    ...
    footprint: Mapped[Any] = mapped_column(geometry_type("GEOMETRY"))
...
class Event(TimestampMixin, Base):
    ...
    geometry: Mapped[Any] = mapped_column(geometry_type("GEOMETRY", spatial_index=True))
