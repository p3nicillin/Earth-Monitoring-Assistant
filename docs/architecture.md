...
The demo detector is intentionally separate and runs only for `provider=demo`. A production
detector must expose its name and immutable version, validate required bands/quality assets, and
return evidence sufficient to reproduce or review the output.
